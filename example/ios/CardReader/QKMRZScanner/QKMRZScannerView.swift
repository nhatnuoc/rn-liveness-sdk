...
public class QKMRZScannerView: UIView {
    ...
    fileprivate func initCaptureSession() {
        ...
        guard let camera = AVCaptureDevice.bestForVideo else {
...
extension QKMRZScannerView: AVCaptureVideoDataOutputSampleBufferDelegate {
    public func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        let detectTextRectangles = VNDetectTextRectanglesRequest { [unowned self] request, error in
            ...
            let mrzTextRectangles = results.map({ $0.boundingBox.applying(transform) }).filter({ $0.width > (imageWidth * 0.6) })
            //This 0.8 value meaning only do OCR on document images that has width >= 80% of full width.
...
extension AVCaptureDevice {
    ...
    static var bestForVideo: AVCaptureDevice? {
        var deviceHasUltraWideCamera = !AVCaptureDevice.DiscoverySession(deviceTypes: [.builtInUltraWideCamera], mediaType: .video, position: .back).devices.isEmpty
        deviceHasUltraWideCamera = false
